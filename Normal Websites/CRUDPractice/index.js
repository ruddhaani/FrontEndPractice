    ...
    for (let i = 0; i < employees.length; i++) {
        ...
        let innerHTMLString = `
            ...
            <td>
                ...
                <button onclick="UpdateEmployee(0, ${employee.employeeId}, ${i})">Update</button>
            </td>
        `;

        if (index >= 0 && index < employees.length && index === i) {
            innerHTMLString = `
                <td><input type="text" value="${employee.employeeId}" readonly></td>
                <td><input type="text" value="${employee.name}" id="updatedName"></td>
                <td><input type="text" value="${employee.department}" id="updatedDepartment"></td>
                <td><input type="number" value="${employee.salary}" id="updatedSalary"></td>
                <td>
                    <button onclick="DeleteEmployee(${employee.employeeId})">Delete</button>
                    <button onclick="UpdateEmployee(1, ${employee.employeeId}, ${i})">Update</button>
                </td>
            `;
        }

        employeeTR.innerHTML = innerHTMLString;
...
async function UpdateEmployee(flag, id, index) {
    if (flag === 0) {
        ...
        let _department = document.getElementById("updatedDepartment").value;
        let _salary = Number(document.getElementById("updatedSalary").value);

        await updateEmployeeInDB(id, _name, _department, _salary);
        await ShowEmployees();
    }
}
